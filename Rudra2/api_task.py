...
def get_universe_info(date):
    ...
    if response.status_code == 200:
        ...
        Say(f"On {date}, the main topic of our universe was: {title}\n{explanation}\nImage URL: {image_url}")
    else:
        Say("Sorry, I could not retrieve information about the universe for that date.")

def ApiExecution(tag,query):
    if "asteroid_count" in tag:
        Api_Key = "dFVeOEqeeJijlEJWyRUks6emx10P5JZfwKMiv7jU"

        Say("Be sure that the starting and ending date range must be under the 7 days")
        ...
        try:
            ...
            if magnitudes:
                Say(f"The brightest asteroid was {min_asteroid_name} with absolute magnitude of: {min_magnitude}")
                Say(f"The most dimmed asteroid was {max_asteroid_name} with absolute magnitude of: {max_magnitude}")
            else:
                Say("No asteroids found.")
        except:
